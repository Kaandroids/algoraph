  ...
  MATRIX: [
    NODE_PROPS,
    {
      ...
      members: [
        ...
        { sig: 'M.setLabels(list rows, list cols?)', desc: 'Name all row (and column) headers shown in the panel, e.g. M.setLabels(nodes()). One list labels both axes.', returns: 'void', cost: 'O(R + C)' },
        { sig: 'M.setLabel(int i, value name)', desc: 'Name a single header i for both axes — pass a vertex to use its name, e.g. M.setLabel(idx[v], v). Unset headers fall back to the index.', returns: 'void', cost: 'O(1)' },
        { sig: 'M.setRowLabel(int i, value name) / M.setColLabel(int j, value name)', desc: 'Name just one axis header.', returns: 'void', cost: 'O(1)' },
      ],
    },
  ],
};
  ...
  groups: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: 'spotlight(structure ds) / spotlight(string name)', desc: 'Emphasise an entry in the Data panel — pass a structure, or a variable’s name. Persists until unspotlight(...) or clearMarks().', returns: 'void' },
        { sig: 'unspotlight(structure ds) / unspotlight(string name)', desc: 'Remove a panel spotlight.', returns: 'void' },
        { sig: 'note(structure ds, string text) / note(string name, string text)', desc: 'Pin a short note onto a panel entry, e.g. note(pq, "extract-min"). Empty text clears it.', returns: 'void' },
        { sig: 'pin(structure ds) / pin(string name)', desc: 'Float a structure or variable to the top of the Data panel. Persists through clearMarks(); unpin(...) restores its place.', returns: 'void' },
        { sig: 'unpin(structure ds) / unpin(string name)', desc: 'Unpin a panel entry.', returns: 'void' },
    ...
    },
    {
      ...
      members: [
        { sig: 'scratch.createList(string name?)', desc: 'A hidden List for private bookkeeping — never drawn on the canvas or shown in the run data panel.', returns: 'List', cost: 'O(1)' },
        { sig: 'scratch.createStack(string name?)', desc: 'A hidden Stack — off-canvas and untracked.', returns: 'Stack', cost: 'O(1)' },
        { sig: 'scratch.createQueue(string name?)', desc: 'A hidden Queue — off-canvas and untracked.', returns: 'Queue', cost: 'O(1)' },
        { sig: 'scratch.createSet(string name?)', desc: 'A hidden Set — off-canvas and untracked.', returns: 'Set', cost: 'O(1)' },
        { sig: 'scratch.createMap(string name?)', desc: 'A hidden Map — off-canvas and untracked.', returns: 'Map', cost: 'O(1)' },
        { sig: 'scratch.createPQueue(string name?)', desc: 'A hidden priority queue — off-canvas and untracked.', returns: 'PQueue', cost: 'O(1)' },
        { sig: 'scratch.createMatrix(int rows, int cols, string name?)', desc: 'A hidden rows×cols matrix of zeros — off-canvas and untracked.', returns: 'Matrix', cost: 'O(R·C)' },
      ],
    },
    {
      title: 'Panel structures',
      members: [
        { sig: 'panel.createList(string name?)', desc: 'An off-canvas List that still appears in the run data panel — drawn nowhere, but watchable.', returns: 'List', cost: 'O(1)' },
        { sig: 'panel.createStack(string name?)', desc: 'An off-canvas Stack, watchable in the run data panel.', returns: 'Stack', cost: 'O(1)' },
        { sig: 'panel.createQueue(string name?)', desc: 'An off-canvas Queue, watchable in the run data panel.', returns: 'Queue', cost: 'O(1)' },
        { sig: 'panel.createSet(string name?)', desc: 'An off-canvas Set, watchable in the run data panel.', returns: 'Set', cost: 'O(1)' },
        { sig: 'panel.createMap(string name?)', desc: 'An off-canvas Map, watchable in the run data panel.', returns: 'Map', cost: 'O(1)' },
        { sig: 'panel.createPQueue(string name?)', desc: 'An off-canvas priority queue, watchable in the run data panel.', returns: 'PQueue', cost: 'O(1)' },
        { sig: 'panel.createMatrix(int rows, int cols, string name?)', desc: 'An off-canvas rows×cols matrix of zeros, watchable in the run data panel.', returns: 'Matrix', cost: 'O(R·C)' },
      ],
    },
    {
      title: 'Language',
